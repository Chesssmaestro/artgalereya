...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans pb-20">
      {/* Language Switcher */}
      <div className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-b border-gray-200 z-50 py-3 px-6 flex justify-center gap-8 shadow-sm">
      ...
      <div className="max-w-4xl mx-auto pt-20 px-4 space-y-12">
        ...
        {/* Page 1: Cover */}
        <section className="bg-white shadow-md overflow-hidden flex flex-col w-full" style={{ minHeight: '800px' }}>
          ...
          <div className="flex-1 bg-[#1A3A5A] relative flex items-center justify-center p-8 overflow-hidden min-h-[500px]">
            ...
            <div className="absolute top-10 right-12 text-white text-3xl md:text-5xl italic font-light tracking-wide z-10">
              {t.dates}
            </div>
          </div>
          ...
          <div className="bg-white px-12 py-16 flex flex-col justify-center border-t border-gray-100">
            <h1 className="text-5xl md:text-7xl italic font-light tracking-wide text-gray-900 mb-4">{t.title}</h1>
            <h2 className="text-3xl md:text-4xl italic font-light tracking-widest text-gray-800 whitespace-pre-line">{t.artist}</h2>
          </div>
        </section>
        ...
        <section className="bg-white shadow-md p-12 w-full text-gray-900 flex flex-col">
          <div className="flex flex-col md:flex-row gap-12 mb-16">
            ...
            <div className="md:w-1/3 shrink-0">
               ...
               <div className="mt-6">
                 ...
                 <p className="text-gray-600 mt-1">{t.artistLocation}</p>
               </div>
            </div>
          ...
          <div className="mt-auto pt-12 border-t border-gray-200">
             ...
             <div className="flex flex-wrap items-center justify-between gap-8 opacity-60 grayscale">
                <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-blue-800"></div><span className="text-xs font-bold w-32">NATIONAL COMMITTEE ON ECOLOGY AND CLIMATE CHANGE</span></div>
                <div className="text-xl font-bold font-serif tracking-tighter">prohelvetia</div>
                <div className="flex items-center gap-2"><div className="w-8 h-8 bg-red-600 text-white flex items-center justify-center font-bold text-xl">+</div><span className="text-xs w-48">Swiss Agency for Development and Cooperation SDC</span></div>
                <div className="w-16 h-16 rounded-full border-2 border-green-700 flex items-center justify-center text-[8px] text-center text-green-800 font-bold">GREEN<br/>UNIVERSITY</div>
...
function WorkRow({ work, tall = false }: { work: Work, tall?: boolean }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 mb-16">
      <div className={`shrink-0 ${tall ? 'md:w-[220px]' : 'md:w-[320px]'}`}>
        <WorkImage work={work} tall={tall} />
        <h3 className="uppercase italic font-bold mt-3 tracking-wide">{work.title}</h3>
        <p className="text-sm text-gray-700">{work.meta}</p>
      </div>
      {work.desc && (
        <div className="flex-1">
          <p className="leading-relaxed text-gray-800">{work.desc}</p>
        </div>
      )}
    </div>
  );
